...
d_mean %>% 
  ...
  ggplot() +
  aes(
    month, mean_sent, color = subsection) +
  stat_summary(fun= 'mean', geom = 'line') + facet_wrap(~subsection)

```
```{r}
#loading package which can handle the R date format POSIXct and POSIXt 
...
geom_line() + 
  scale_x_discrete(
    ...
    labels = months_labs) +
  labs(
    title = 'Average sentiment compound score per month per subsection',
    x = '',
    y = 'Average sentiment compound score'
  ) #+ 
...
d_test %>% filter(
  ...
  count()
```
Yeah, we don't have a lot of data points per subsection per month. 
