...
# real deal - will probably take about 3 hours to run
...
```{r}
write_csv(df_complete, 'df_complete.csv')
```
...
```{r}
df_complete %>% filter(
  type == 'News' | type == 'Orbituary (Obit)' | type == 'briefing'
  ) %>% ggplot() +
  aes(subsection, headline_sentiment_comp) +
  stat_summary(fun = mean, geom = 'point')
```
...
m1 <- lm(headline_sentiment_comp ~ subsection, df_complete)
summary(m1)
